...
class OAuth2 {
  constructor(authDelegate) {
    ...
    this.authenticators = {
      [AUTH_TYPE.BASIC]: new BasicAuthenticator((clientId, clientSecret, done) => {
        authDelegate
          .findClient({
            clientId,
            clientSecret
      ...
      [AUTH_TYPE.BEARER]: new BearerAuthenticator({ passReqToCallback: true }, (req, accessToken, done) => {
        return authDelegate
          .findUserByToken({
            accessToken,
            params: req.params
      ...
      [AUTH_TYPE.CLIENT]: new ClientAuthenticator((clientId, clientSecret, done) => {
        authDelegate
          .findClient({
            clientId,
            clientSecret
