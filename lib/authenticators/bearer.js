...
class BearerAuthenticator extends Authenticator {
  ...
  authenticate(req) {
    ...
    if (req.headers && req.headers.authorization) {
      ...
      if (parts.length === 2) {
        ...
        return oauthifizerScope.fail(
          HTTP_STATUSES.BAD_REQUEST.code, 'Invalid bearer token provided');
      }
    }

    if (req.body && req.body.access_token) {
      if (token) {
        return oauthifizerScope.fail(HTTP_STATUSES.BAD_REQUEST.code, 'Multiple tokens provided');
      }
    ...
    if (!token) {
      return oauthifizerScope.fail(HTTP_STATUSES.BAD_REQUEST.code, 'No token provided');
    }
    ...
    function callback(err, user, info = {}) {
      ...
      if (!user) {
        ...
        return oauthifizerScope.fail(HTTP_STATUSES.BAD_REQUEST.code, 'invalid_token');
      }
